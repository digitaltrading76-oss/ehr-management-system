...
from report_engine import build_assessment_context, create_docx_report
...
def central_case_detail(case_id:str, request:Request):
    ...
    checklist="".join([f"<tr><td>{x['item']}</td><td><span class='status-ok'>✅ Complete</span></td></tr>" if x.get("status") else f"<tr><td>{x['item']}</td><td><span class='status-missing'>❌ Missing / Pending</span></td></tr>" for x in ctx["checklist"]])
    policy_rows="".join([f"<tr><td>{x.get('policy_category','')}</td><td>{x.get('possible_violation','')}</td><td>{x.get('policy_basis','')}</td><td>{x.get('usual_next_step','')}</td></tr>" for x in ctx["policy"]])
    labor_rows="".join([f"<tr><td>{x.get('title','')}</td><td>{x.get('quote','')}</td><td>{x.get('caution','')}</td></tr>" for x in ctx["labor_refs"]])
    missing_rows="".join([f"<li>{m}</li>" for m in ctx.get("missing",[])]) or "<li>No missing requirement identified.</li>"
    evidence_rows="".join([f"<tr><td>{d['document']}</td><td>{d['status']}</td><td>{d['remarks']}</td></tr>" for d in ctx["documents"]])
    facts_list="".join([f"<li>{fact}</li>" for fact in ctx["facts"]])
    action_list="".join([f"<li>{act}</li>" for act in ctx["recommended_actions"]])
    files="".join([f"<li><a href='/case-file/{case_id}/{f}'>{f}</a></li>" for f in case.get("uploaded_files",[])]) or "<li>No uploaded file</li>"
    scores=ctx.get("scores",{})
    ev_level, ev_explanation = ctx["evidence_level"]
    ...
    return render("case_detail.html",request,{
        ...
        "EXTRACTED_PREVIEW":ctx.get("extracted_preview",""),
        "DUE_PROCESS":case.get("due_process_score",0),
        "READINESS":case.get("overall_readiness",0),
        "STATUS":case.get("status",""),
        "NEXT_MOVE":ctx.get("recommended_next_move",""),
        "CHECKLIST":checklist,
        ...
        "SAFEGUARD":ctx.get("safeguard","No final HR action should be recommended until the worker has been properly notified and given an opportunity to respond."),
...
@app.get("/case/{case_id}/download-assessment-docx")
def download_assessment_docx(case_id:str, request:Request):
    if not is_central(request): return RedirectResponse("/access-denied",302)
    case=next((c for c in load_cases() if c["case_id"]==case_id),None)
    if not case: return HTMLResponse("Case not found",404)

    safe_case=case_id.replace("/","_").replace("\\","_")
    output_path=REPORT_DIR/f"{safe_case}_EHR_Case_Assessment_Report.docx"
    create_docx_report(case, output_path)
    return FileResponse(output_path, filename=f"{safe_case}_EHR_Case_Assessment_Report.docx", media_type="application/vnd.openxmlformats-officedocument.wordprocessingml.document")
